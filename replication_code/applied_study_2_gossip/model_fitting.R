...
filtered_df <- read.csv("./data-raw/gossip/filtered_df_no_missing_wave_4.csv")
...
summary <- read.csv(".data-raw/gossip/summary.csv")

# pb <- tkProgressBar(max=length(class_ids))
# progress <- function(n) setTkProgressBar(pb, n)
# opts <- list(progress=progress)

cl <- parallel::makeCluster(NUM_CORE, outfile="LogGossip.txt")  
