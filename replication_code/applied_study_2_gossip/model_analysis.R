...
load("wave4_meta_data.RData")
load("wave4_meta_results.RData")
...
#transform correlation estimates to fisher's z
pre_process_corr <- function(model_res) {
    model_res$mean <- DescTools::FisherZ(model_res$mean)
...
aggregate_results <- function(model_res, priors, class = class_ids, is_corr = FALSE) {
    ...
    for (i in 1:length(params)) {
        ...
        print(mean(dat$mean))
        if (is_corr) {
            m.brm <- brm(mean ~ 1 + (1|class), data = dat, prior = priors, iter = 5000)
        } else {
...
post_process_corr <- function(model_res) {
    model_res$Estimate <- DescTools::FisherZInv(model_res$Estimate)
    model_res$'l-95% CI' <- DescTools::FisherZInv(model_res$'l-95% CI')
    model_res$'u-95% CI' <- DescTools::FisherZInv(model_res$'u-95% CI')
    model_res$Est.Error <- NULL
    return(model_res)
}
##META ANALYSIS##
...
corr_priors <- c(prior(normal(0, 10), class = Intercept))
...
m1Corr = aggregate_results(model1Corr, corr_priors, is_corr = TRUE)
m1Corr <- post_process_corr(m1Corr)

m2Corr = aggregate_results(model2Corr, corr_priors, is_corr = TRUE)
